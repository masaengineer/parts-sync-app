...
海外向け EC 販売に 6 年間従事した経験から、ニッチな市場には SaaS が行き届いていないこと実感していました。労働集約型ビジネスモデルでは、事業主の時間や体力のリソース制限が業績の上限となるケースが数多く存在します。
...
年商 3,000 万円〜数億円規模の海外向け EC 事業者<br>
...
「Parts Sync」は上記の課題に対し、以下の解決策を提供します。

1.  **データの一元管理：**
最適化されたデータベースを導入し、分散していた商品代金、手数料、送料、外注費などの情報を一元管理します。これにより、データの整合性を保ち、更新漏れを防ぎます。
2.  **使いやすい Web アプリケーション：**
    - 情報共有の促進： Web アプリ形式で提供することで、複数スタッフ間でのリアルタイムな情報共有を容易にします。
    - 優れた UI/UX： UI 設計においては『ノンデザイナーズ・デザインブック』等で提唱されているデザイン原則（近接、整列、反復、コントラスト）を意識しました。これにより、直感的で一貫性のあるインターフェースを実現し、複雑さを排除することでユーザーの認知負荷を低減し、誰にとっても使いやすいアプリケーションを目指しました。
3.  **大幅な工数削減とコストカット：**
上記により、日次の煩雑な集計作業にかかる工数を大幅に削減し、**年間 100 万円規模のコストカット**\*を実現します。（※時間単価 3 千円、月 30 時間削減の場合の試算）
...
当初、この同期処理を App サーバープロセス内で直接実行（同期的処理）することを試みました。しかし、eBay からのデータ取得・処理には 3〜5 分程度を要するため、App サーバーのリクエスト処理スレッドを長時間占有してしまい、ユーザーへのレスポンス遅延や、場合によってはリソース枯渇によるサーバーダウンを引き起こしてしまうことがありました。
当初、この同期処理を App サーバープロセス内で直接実行（同期的処理）することを試みました。しかし、eBay からのデータ取得・処理には 3〜5 分程度を要するため、App サーバーのリクエスト処理スレッドを長時間占有してしまい、ユーザーへのレスポンス遅延や、場合によってはリソース枯渇によるサーバーダウンを引き起こしてしまうことがありました。

この問題を解決するため、**App サーバーの役割（リクエスト処理）と重たいバックグラウンド処理を明確に分離**する方針としました。Sidekiq を導入することで、以下のメリットが得られました。
この問題を解決するため、**App サーバーの役割（リクエスト処理）と重たいバックグラウンド処理を明確に分離**する方針としました。Sidekiq を導入することで、以下のメリットが得られました。

1.  **非同期実行による UX 向上:** 重たい同期処理をバックグラウンドに移行し、App サーバーは同期的なリクエスト処理に専念できるため、ユーザーは待たされることなく快適にアプリケーションを利用できます。
2.  **堅牢性の向上:** Sidekiq が標準で提供するリトライ機能により、ネットワークや API の一時的なエラーが発生しても自動で再試行され、ジョブの成功率が高まります。
3.  **スケーラビリティの確保:** 将来的に処理対象データやユーザー数が増加した場合でも、App サーバーとは独立して Sidekiq ワーカーの数を調整することで、柔軟に性能をスケールさせることが可能です。
4.  **スケーラビリティの確保:** 将来的に処理対象データやユーザー数が増加した場合でも、App サーバーとは独立して Sidekiq ワーカーの数を調整することで、柔軟に性能をスケールさせることが可能です。

同期処理の負荷軽減策としてデータ期間制限や App サーバーのスケールアップも考慮しましたが、App サーバーのブロッキング問題や、一体化した構成での非効率なスケーリング（リソース・コスト面の限界）という根本課題は残ります。そこで、App サーバーとワーカサーバーを分離し独立してスケールできる非同期処理構成を採用し、長期的なパフォーマンスと運用性を重視しました。<br>
...
2.  **直感的でシンプルな UI/UX:** 検索条件をユーザーが簡単に指定できるよう、アイコンやプレースホルダーを効果的に配置しました。なお、**日付範囲選択は、ユーザーの主な販路である eBay のインターフェースを模倣することで、認知負荷を減らすこと**を意識しました。また、一般的な選択肢（「今日」「今週」「先月」など）の場合は即座に検索を実行し、**具体的な日付を指定したい場合にのみ「カスタム」を選択すると、日付入力フィールドが表示される**という設計にすることで、不要な要素を排除し、シンプルで直感的な操作感を実現しました。（Stimulus コントローラーによって制御）
...
過去に既製品 SaaS を導入した際、多機能であるがゆえに操作が複雑で、長いマニュアルを読んだ時点で利用意欲が削がれてしまった経験があります。優れた機能性があることは前提ですが、toB ツールにおいても toC に劣らない UIUX を提供することは重要だと考えています。特に**新規ユーザーがスムーズに操作方法を習得し早期に価値を実感できるようにすることは、ツールの定着と離脱率低下のために非常に重要** だと考えました。
...
この機能を通じて、**新規ユーザーの認知負荷を効果的に下げ、操作への心理的ハードルを取り除く**ことで、サービスへのスムーズな導入を促し、結果として**ユーザー定着率の向上**を目指しています。
<br>
...
| **データベース**   | PostgreSQL                                         |
| **インフラ**       | AWS (ECS/Fargate, ECR, RDS, Internet Gateway, ALB, S3, Route53)             |
| **キャッシュ**     | Redis                                              |
...
本プロジェクトは**コスト効率と運用性のバランス**を重視したインフラ構成を採用しています。

- **リソースの最適化:** 主要コンポーネント（RDS データベース、ECS タスク）を単一 AZ に集中配置することで、運用コストを削減しています。ALB は技術要件上複数 AZ のサブネット設定が必須ですが、実際のアプリケーションリソースは単一 AZ にのみ配置しています。

- **マネージドサービスの活用:** EC2 インスタンスの直接管理ではなく Fargate を採用し、ECR や S3 などのマネージドサービスと VPC エンドポイントを組み合わせることで、運用負担を軽減しながらもセキュアな環境を実現しています。

- **拡張性の確保:** 現在はコスト効率を優先した構成ですが、ユーザー基盤の拡大に応じて複数 AZ 構成への段階的な移行が可能な設計としています。

このようにビジネスの現段階に適したインフラ構成を採用することで、初期コストを抑えながらも将来の成長に対応できる柔軟性を確保しています。
