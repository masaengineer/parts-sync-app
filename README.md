...
販売情報を一覧表示して、商品あたりの限界利益（売上高-変動費）を確認することができます。より詳細な手数料の内訳を確認したい場合は、各商品行をクリックすることで詳細画面がモーダル表示されます。
...
月ごとに集計したデータをグラフ形式とテーブル形式でそれぞれ確認して、視覚的に年度ごとの業績を一目で把握できるダッシュボード機能です
...
CSV 形式のインポートでは委託先から入手した外注費を DB へ保存します。また、基本情報はプラットフォームの API 経由で自動的に更新されます。Sidekiq+Redis を用いて 1 時間ごとの定期実行を実現しました。
...
Ransack を利用した検索機能です。販売データを日付、注文番号、追跡番号、SKU コードでマルチ検索することができます。アイコンやプレースホルダーを利用することで、情報量を最小限にしつつ、直感的な UI を実現しています。
