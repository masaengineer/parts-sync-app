...
RSpec.describe 'ページネーション機能', type: :system do
  ...
  describe '商品一覧のページネーション' do
    it 'ページネーションが表示され、ページを切り替えられる', js: true do
  ...
  describe '検索結果のページネーション' do
    before do
      # 検索用のデータを準備
      create_list(:seller_sku, 20, sku_code: 'TEST-SKU')
    end

    it '検索結果にもページネーションが適用される', js: true do
      visit seller_skus_path
